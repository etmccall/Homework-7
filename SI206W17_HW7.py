...
for tweet in umsi_tweets: 
	tupz=(None, tweet["id"], "umsi", tweet['created_at'], tweet["text"], tweet['retweet_count'])
...
quer="SELECT time_posted FROM Tweets"
cur.execute(quer)
...
def get_twitter_users(z): 
	g=z.split()
	return {re.findall("@([0-9A-Za-z_]+)",m)[0] for m in g if re.search("@[0-9A-Za-z_]+",m)}	
